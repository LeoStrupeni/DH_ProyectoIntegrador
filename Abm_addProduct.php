...
if($_SERVER['REQUEST_METHOD']=='POST'){ 
    ...
    $query="SELECT `ID` FROM `prod_marcas` where `Nombre` = :marca";
    $sentencia=$pdo->prepare($query);
    $sentencia->bindparam(':marca',$_POST['marca']);   
    $sentencia->execute(); 
    $idMarca=$sentencia->fetch(PDO::FETCH_ASSOC);

    $query="SELECT ID FROM `prod_categorias` WHERE `Nombre` = :subcategoria";
    $sentencia=$pdo->prepare($query);
    ...
    'INSERT INTO `productos_usuarios`
        (`ID`, `IdUsuario`, `Name`, `Descripcion`, `Precio`, `Graduacion`, `Origen`, `imagen`, 
    ...
    ':MARCA'=>$idMarca['ID'],
    ':CATEGORIA'=>$idSubcategoria['ID']
    ));

    $query="SELECT max(`ID`) as ID FROM `productos_usuarios`";
